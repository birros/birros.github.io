...
const setupMouseTrail = (el: HTMLDivElement | null) => {
  if (!process.browser) {
    return
  }
  ...
  gl.clearColor(...colorToRgbArray(getComputedStyle(el).backgroundColor))
  ...
  el.addEventListener('transitionend', resize)
  resize()
  ...
  return () => {
    ...
    el?.removeEventListener('transitionend', resize)
    window.removeEventListener('mousemove', updateMouse, false)
    gl.canvas.parentElement?.removeChild(gl.canvas)
  }
}

export default setupMouseTrail
